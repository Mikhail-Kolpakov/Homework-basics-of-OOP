...
void calculating(int array[], int length, float& f_par, float& s_par, float& t_par, float f_for(float), float s_for(float), float t_for(float), TypeOfArray type_of_array) { //Функція для обрахунку трьох вхідних параметрів
    float s = 0, p = 1;
    for (int i = 0; i < length; i++) {
        s += array[i];
        p *= array[i];
